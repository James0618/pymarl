...
class NewLearner:
    ...
    def train_transition(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length - 1):
            _, next_hidden_state, pred_observation, pred_reward = self.mac.get_agent_outs(batch, t=t)
        ...
        # Concat over time
        pred_obs, pred_rs = th.stack(pred_obs, dim=1), th.stack(pred_rs, dim=1)
        ...
        if index.shape[0] != 0:
            ...
            loss = masked_obs_loss + pred_sum_rs_loss + nonzero_loss

        else:
            nonzero_loss = 0
            # Normal L2 loss, take mean over actual data
            loss = masked_obs_loss + pred_sum_rs_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            self.logger.log_stat("transition_loss", loss.item(), t_env)
    ...
    def train_value(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Concat over time
        cur_q_out = th.stack(cur_q_out, dim=1)
